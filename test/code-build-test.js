...
describe("inputs2Parameters", () => {
  ...
  it("build override parameters for codeBuild.startBuild", () => {
    ...
    const test = inputs2Parameters({
      ...
      imageOverride: "111122223333.dkr.ecr.us-west-2.amazonaws.com/codebuild-docker-repository"
    });
    expect(test).to.haveOwnProperty("projectName").and.to.equal(projectName);
    expect(test).to.haveOwnProperty("sourceVersion").and.to.equal(sha);
    expect(test)
    ...
    expect(test)
      .to.haveOwnProperty("buildspecOverride")
      .and.to.equal(undefined);
    expect(test).to.haveOwnProperty("computeTypeOverride").and.to.equal(`BUILD_GENERAL1_LARGE`);
    expect(test).to.haveOwnProperty("environmentTypeOverride").and.to.equal(`LINUX_CONTAINER`);
    expect(test).to.haveOwnProperty("imageOverride").and.to.equal(`111122223333.dkr.ecr.us-west-2.amazonaws.com/codebuild-docker-repository`);
